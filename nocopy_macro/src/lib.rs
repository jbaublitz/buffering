...
//! * The type of each field is stack-allocatable and the size is known at compile time, or in
//! other words, the size does not change
//!
//! # Provided methods
...
//! * The union also provides methods `.get_your_field_name` and `.set_your_field_name` that are
//! generated per struct field (and where `your_field_name` is replaced by the actual name of the
//! field)
//! * Each getter and setter will respect endianness set by the corresponding field attribute
//! provided in the original struct
//!
//! # Recognized attributes
//! Attributes can be added to fields to specify whether they should be interpreted as big endian values
//! or little endian values in the form `#[endian = "big"]` or `#[endian = "little"]`. If neither
//! is specified, native endian is assumed. It is provided on a per-field basis due to restrictions
//! on what types provide the `to_be`, etc. methods. These fields should be used when doing
//! internet networking to assure cross-architecture portability. Protocols that assume native
//! endian buffer representation do not need these attributes. A struct-level attribute is also
//! provided as `#[name = "MyUnionNameHere"]` to override the autogenerated name for the resulting
//! union.
